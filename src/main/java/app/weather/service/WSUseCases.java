...
public class WSUseCases {
	...
	private void initWebService() {
		try {
			...
			AwdbWebService_Service lookup = new AwdbWebService_Service(wsURL,
					new QName("http://www.wcc.nrcs.usda.gov/ns/awdbWebService", "AwdbWebService"));
			m_webService = lookup.getAwdbWebServiceImplPort();
		} catch (Exception e) {
			System.out.println("Problem creating web service client: " + e.getMessage());

		}
	}

	/**
	 * Example main program entry point which: 1 - instantiates this class and
	 * creates a web service object 2 - gets a list of stations, loop through
	 * the stations and print out each station's metadata 3 - uses Use Case 3 to
	 * get last 7 days data for each station and print out each day's date and
	 * values.
	 * 
	 * @param args
	 */
	...
	public static void main(String[] args) {
		...
		String stationName, stationTriplet, beginDate, endDate, countyName, huc, shefID, fipsCountryCode,
				fipsCountyCode, fipsStateNumber;
		...
		for (StationMetaData metadatum : metadata) {
			...
			if(fipsCountryCode.equalsIgnoreCase("US")){
			...
			/*
			 * Use Case 3: Get Last 7 Days Data for One Station This will get
			 * DAILY data for each of the stations from Use Case 1. Note: To
			 * apply Use Case 2: Period of Record of Data for One Station, see
			 * example method getPeriodOfRecord().
			 */
...
//			BigDecimal[] stationValues = data[0].getValues().toArray(new BigDecimal[0]);
//			// Get values for current station
//			if (stationValues != null && stationValues.length > 0) {
//				for (int i = 0; i <= stationValues.length; i++) {
//					System.out.println("Date: Flags/Value: " + beginDt + ":  " + flags + "/" + stationValues[i] + '\n');
//				}
//			}
		}
	}
	...
	/**
	 * Use Case 1: Get Inventory of Stations This example will get an inventory
	 * of all stations in Oregon for SNOTEL stations that have Snow Water
	 * Equivalent and return list of stations
	 */
	...
	public List<StationMetaData> getStations() {
		...
		List<String> stationTriplets = m_webService.getStations(stationIds, stateCds, 
				networkCds, hucs, countyNames,
				minLatitude, maxLatitude, minLongitude, 
				maxLongitude, minElevation, maxElevation, elementCodes,
				ordinals, heightDepths, logicalAnd);
		List<StationMetaData> stations = m_webService.getStationMetadataMultiple(stationTriplets);
		//Forecast averagesPeakData = m_webService.getForecast(stationTriplets.get(0), null, null, null);
		//System.out.println(averagesPeakData.getCalculationDate() + averagesPeakData.getElementCd() + averagesPeakData.getForecastPeriod() + averagesPeakData.getPublicationDate() + averagesPeakData.getUnitCd());
		return stations;
	}

	/**
	 * Use Case 2: Get period of record for one station. This will return period
	 * of Data that are SNOW WATER EQUIVALENT (element code = WTEQ) for a given
	 * station and date range. Note: Always use an ordinal of 1, and heightDepth
	 * of null (height depth is only used for soil sensors)
	 * 
	 * @param p_stationTriplet
	 *            The station to get data for, ex: "471:ID:SNTL"
	 * @param p_beginDate
	 *            The begin date - a String format "yyyy-MM-dd"
	 * @param p_endDate
	 *            The end date - a String format "yyyy-MM-dd"
	 * @return An Array of Data Objects
	 */

	public Data[] getPeriodOfRecord(String p_stationTriplet, String p_beginDate, String p_endDate) {
		Data[] values = m_webService.getData(Arrays.asList(p_stationTriplet), "WTEQ", 1, null, Duration.DAILY, true,
				p_beginDate, p_endDate, true).toArray(new Data[0]);
		return values;
	}

	/**
	 * Use Case 3: Get last seven days of data. This will return the last seven
	 * days of SNOW WATER EQUIVALENT (element code = WTEQ data, relative to
	 * today, for a given station. Note: Always use an ordinal of 1, and
	 * heightDepth of null (height depth is only used for soil sensors)
	 * 
	 * @param p_stationTriplet
	 *            The station to get data for, ex: "471:ID:SNTL"
	 * @return An Array of Data Objects
	...
	public Data[] getLastSevenDaysData(String p_stationTriplet) {
		...
		Data[] values = m_webService.getData(Arrays.asList(p_stationTriplet), "WTEQ", 1, null, Duration.DAILY, true,
				sevenDaysAgo, today, true).toArray(new Data[0]);
